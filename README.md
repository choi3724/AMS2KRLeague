...
1. [Latest Release](https://github.com/choi3724/AMS2KRLeague/releases/latest)에서 `AMS2-League-Overlay-0.4.5-Setup.exe`를 다운로드하고 실행합니다.
...
설치 프로그램을 사용하지 않으려면 `AMS2-League-Overlay-0.4.5-win-x64.zip`을 원하는 폴더에 풀고 `AMS2LeagueClient.exe`를 실행해도 됩니다. 별도 .NET 설치나 명령줄 설정은 필요하지 않습니다.
...
- 0.4.0 이하에는 이 기능이 없으므로 **0.4.5를 한 번 수동 설치**해야 합니다. 커밋만으로 업데이트가 배포되지 않으며 GitHub 릴리스에 새 설치 파일이 게시되어야 합니다.
...
첫 공개 버전은 `0.1.0`, 현재 개발 버전은 `0.4.5`, 안정 기준선은 `0.2.2`입니다. 자세한 규칙은 [VERSIONING.md](VERSIONING.md)를 참고하십시오.
