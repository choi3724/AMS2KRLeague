...
1. [Latest Release](https://github.com/choi3724/AMS2KRLeague/releases/latest)에서 `AMS2-League-Overlay-0.4.0-Setup.exe`를 받습니다.
...
설치 프로그램을 사용하지 않으려면 `AMS2-League-Overlay-0.4.0-win-x64.zip`을 원하는 폴더에 풀고 `AMS2LeagueClient.exe`를 실행해도 됩니다. 별도 .NET 설치나 명령줄 설정은 필요하지 않습니다.
...
타워 시간은 각 참가자의 AMS2 **최고 랩 기록**입니다. 연습·예선에서 기록이 없으면 출차 후 `아웃랩` → 계측 시작 후 `랩 타임 주행 중` → 유효 기록 인정 후 최고 기록으로 바뀝니다. 이미 최고 기록이 있으면 다음 주행에도 유지합니다. 레이스는 첫 바퀴 `아웃랩`, 두 번째 바퀴 `레이스 중`, 두 번째 바퀴 완료 이후 유효한 AMS2 최고 기록을 표시합니다. `PIT`, `FIN`, `RET`, `DNF`, `DSQ`는 별도 상태로 표시합니다. 개인 현재·섹터 타임은 독립 패널에 유지하며, 랩 무효 신호가 오면 현재 타임을 고정하고 무효를 표시합니다. 과거 유효 최고 기록은 무효 처리하지 않습니다.

전후방 패널은 같은 영역의 차량만 비교합니다. 트랙에서는 트랙 진행거리, 피트에서는 차량 방향과 월드 좌표를 이용한 가까운 차량의 직선거리(`~Nm`)를 표시합니다. 시간차는 실제 선택 차량에 대응하는 유효 AMS2 split이 있을 때만 표시하며, 거리를 시간으로 환산하지 않습니다. `LAP N`은 레이스에서만 표시하고 연습·예선에서는 표시하지 않습니다.
...
첫 공개 버전은 `0.1.0`, 현재 릴리스는 `0.4.0`, 안정 기준선은 `0.2.2`입니다. 자세한 규칙은 [VERSIONING.md](VERSIONING.md)를 참고하십시오.
