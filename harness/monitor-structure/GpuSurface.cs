...
sealed partial class GpuSurface : IDisposable
{
    ...
    public void Dispose() {
        DisposePlot();host.Close();foreach(var x in transient)x.Dispose();foreach(var x in images.Values)x.Dispose();foreach(var x in geometries){if(!x.Key.IsFrozen)x.Key.Changed-=x.Value.Changed;x.Value.Native?.Dispose();}foreach(var x in colors.Values)x.Dispose();
