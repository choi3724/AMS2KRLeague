...
0.4.3부터 게임 실행 중에도 오버레이만 정상 저장·종료·설치·백그라운드 재실행합니다.
업데이트 동안 오버레이와 기록 수집은 잠시 중단됩니다. 게임과 다른 앱은 종료하지 않습니다.
기존 0.4.1~0.4.2에서 0.4.3 이상으로 처음 자동 설치할 때는 이전 버전의 게임 종료 대기가 적용됩니다.
0.4.0 이하에서는 최초 0.4.4 수동 설치가 필요합니다.

현재 지원
- 배치 편집 중 빈 이벤트·레이스 컨트롤의 미리보기 및 패널 내부 전체 선택·이동
- 개별 이동·크기 조절·표시 여부를 저장할 수 있는 한국어 오버레이
- 최고속 랩 상태와 참가자 수에 맞춰 자동으로 줄어드는 타워 높이(최소 2행, 저장된 최대 크기 유지)
