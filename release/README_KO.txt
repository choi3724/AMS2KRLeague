AMS2 League Overlay 0.4.2
============================================

빠른 시작
1. AMS2 옵션 → 시스템 → 공유 메모리에서 Project CARS 2를 선택합니다.
2. AMS2를 테두리 없는 창 또는 창 모드로 실행합니다.
3. AMS2LeagueClient.exe를 실행합니다.

첫 실행 창에서 AMS2, 공유 메모리, 서버, 계정 상태를 확인할 수 있습니다.
별도 .NET 설치, PowerShell, JSON 편집, API 토큰 입력은 필요하지 않습니다.

자동 업데이트
0.4.1부터 GitHub 최신 버전을 자동 확인하고 검증한 설치 파일을 같은 폴더에 적용합니다.
게임 실행 중에는 설치를 미루고 종료 후 정상 저장·설치·재실행합니다. 상태창에서 진행을 확인하세요.
0.4.0 이하에서는 최초 0.4.2 수동 설치가 필요합니다.

현재 지원
- 개별 이동·크기 조절·표시 여부를 저장할 수 있는 한국어 오버레이
- 드라이브스루·스톱 앤 고·실격 페널티 전용 열과 완주/피트 상태의 동시 표시
- 클래스·참가자별 최고 랩·아웃랩 단계를 표시하는 순위 타워와 피트/트랙을 구분하는 앞차/뒤차 UI
- 가로·세로 독립 패널 크기와 찌그러지지 않는 자동 글꼴 배율
- 일반 황색기·이중 황색기·전 코스 황색기를 구분하는 레이스 컨트롤
- 자유 연습·예선·레이스 대기 화면과 게임 접속 로그를 통한 싱글/멀티 자동 판정
...
- AMS2 Shared Memory에는 권위 있는 멀티플레이어 여부 값이 없어 공식 접속 로그로 모드를 판정합니다. 싱글·미확인 기록은 로컬 보관하며 전송하지 않습니다.
- 접속 로그가 없거나 오래되면 모드 미확인으로 보류합니다. 혼자 연 멀티 방의 조용한 로그나 퇴장 경계도 보류될 수 있습니다.
