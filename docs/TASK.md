...
로컬 게이트 PASS: 기준162/162·111/111, 후보165/165·111/111, 최종 배포 DLL의 제공설정WPF2/2, build warning/error0. 격리0.7.5→0.7.6 업데이트/재시작18캡처/사용자 파일 보존·470파일 해시 일치. [보고서](reports/2026-09-15-release-0.7.6.md). 게시 결과는 작업 루트 published-verification.json에 기록.

# REQ-RELEASE-075 — 저장 HUD 크기 수정 공개

사용자의 다음 버전 커밋·릴리즈 승인에 따라 공개0.7.4에서 HUD 크기 수정만 추출한0.7.5를 검증·패키징·커밋·태그·push·Latest 게시한다. 기존 분석/원본 dirty 변경과 설치본은 보존한다. HUD별 물리 크기 기준/호환 백업, 관련 WPF 회귀·전체 회귀·격리 업데이트 및 공개 다운로드를 확인한다. 게임/운영 업로드/기본 renderer 전환 없음. 실게임 및 물리 트리플 NOT TESTED, Monitor RED 유지.

로컬 게이트 PASS: 공개 기준159/159·111/111, 후보162/162·111/111, WPF3/3 및 최종 배포 DLL3/3, build warning/error0. 격리0.7.4→0.7.5 업데이트/재시작18캡처/사용자 파일 보존·470파일 해시 일치. [보고서](reports/2026-09-15-release-0.7.5.md). 공개 결과는 릴리즈 작업 루트의 published-verification.json에 기록한다.

# REQ-RELEASE-074 — 다중 모니터 배치 수정 공개

사용자 승인으로 트리플 모니터 UI 배치 개선을0.7.4로 공개한다.0.7.3의 독립 체크아웃에 배치 관련 변경만 반영하며 기존 개발 트리·미커밋 성능/GPU 작업을 보존한다. 회귀·실제 WPF 저장/복원·패키지 감사·격리 업데이트·commit/tag/push/GitHub Release/다운로드 검증을 수행한다. 설치본·게임·서버는 변경하지 않는다. 물리 트리플/mixed-DPI/실게임 미검증과 Monitor RED를 명시한다.

# REQ-RELEASE-073 — 0.7.3 공개 릴리즈

사용자가0.7.3 릴리즈를 승인했다. 검증한 REQ-N-HEADROOM-11 변경을 보존하고 버전·패키지·격리업데이트·commit/tag/push/GitHub Latest 게시·공개 다운로드 검증까지 진행한다. 기존 설치본·게임·서버·사용자 수동 보정은 변경하지 않는다. Monitor RED 및 실게임/VR 미검증을 유지한다. 이전 단계의 릴리즈 금지는 이번 요청에 한해 해제된다.

# REQ-N-HEADROOM-11 — 레드존 위 최소 1000RPM 여유 (2026-09-13)

자동 최대 눈금을 ceil((redStart+1000)/1000)*1000으로 수정한다. 6800→8000,7000→8000,7000.01→9000,14800→16000. 이전 next-thousand 공식보다 이 항목이 우선한다. 노랑/빨강90%/97% 및 관측 레드존 기준200ms 점멸·외곽5쌍 완등은 유지한다. 저장된 수동 최대값/원본 데이터/렌더 자원·외형을 보존한다. 공통 resolver와 기존 회귀·WPF 캡처/정책 문서만 수정하며, 현재 공개0.7.2/760343a는 변경하지 않는다. 별도 검증 빌드만 준비하고 commit/tag/push/release/설치 교체는 이번 수정에서 하지 않는다. 기준선 work/rpm-headroom. ponytail 미적용.
