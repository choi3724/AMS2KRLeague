...
- 주행 계기판 3종, A/B/C/H 입력 막대, 부드러운 좌측 이동 그래프와 편집 미리보기.
- 색상·글꼴 저장, 한글패치 Pretendard Medium 내장, 지정 앱/설치 아이콘.
- 업데이트 후 상태창 자동 재실행과 실행 실패 판정.
- 연습·예선·레이스 기록을 전체 레이스 종료·최종 저장 후 일괄 전송. 늦은 참가자 실제 시작/공통 종료 기준, 결과 화면 재수집 방지.
- 예선 종료 상태의 레이스 결과 오인 방지 및 트랙이 비워진 스냅샷 수집 차단.

## 0.4.4 — 2026-09-09

- 배치 편집 중 빈 이벤트·레이스 컨트롤에 가상 미리보기 표시. 실제 이벤트 수신 및 편집 종료 시 실제 상태 복원.
- 편집 drag surface에 입력 가능한 배경을 적용해 내부의 마우스 통과 문제 수정. 크기 조절·커서·일반 플레이 클릭 통과 유지.
- 미리보기와 실제 이벤트·기록·업로드 분리. WPF 회귀 테스트 추가, Client 113/113·Activity 102/102 검증.

## 0.4.3 — 2026-09-08

- 타워 베스트랩 상태를 최고속 랩으로 표시.
- 참가자 수에 따른 타워 배경·창 높이 자동 축소와 증가 시 복원. 사용자 최대 크기와 다른 패널 배치 보존.
- 게임 실행 여부로 업데이트를 미루던 세 단계의 조건을 제거. 오버레이 정상 저장·종료 후 설치하고 백그라운드 재실행. 같은 경로 중복 실행 및 무결성 방어 유지.
- 기존 0.4.1~0.4.2에서 최초 업데이트는 구버전의 게임 종료 대기를 따르며, 설치 중 표시·수집 공백이 있음.

## 0.4.2 — 2026-09-08

- AMS2 공식 접속 로그에 기반한 싱글/멀티 자동 판정. 수동 선택 제거와 상태창·대기 오버레이 반영.
- 결과/Witness/리플레이의 멀티 확인 구간만 전송하고 싱글·미확인·혼합 경계 기록은 로컬 보관. 기존 대기열도 검사.
- 결과/Witness raceMode 및 리플레이 사이드카·HTTP 헤더에 MULTIPLAYER/SINGLE_PLAYER/UNKNOWN 전달. Compact 원본 보존.
- 실제 로그 6개 확인, Client 111/111·Activity 102/102 검증. 조용한 멀티 로그 및 세션 경계 보류, 운영 서버 매핑·실게임 전환 미검증 범위 명시.
- 사용자 요청으로 0.4.2 정식 Latest 게시.

## 0.4.1 — 2026-09-08

- 레이스 종료의 FINAL과 타워 완료 상태를 완주로 한글화. 상태 배지·랩 안내·데모·오류·설치 안내를 한글로 표시.
- 타워 우측 페널티 열 추가. 드라이브스루/스톱 앤 고/실격을 원본 필드로 표시하고 피트·완주·최고기록과 독립 갱신.
- 기존 타워 저장 폭 호환 확장 및 업데이트 진행 상태 표시.
- GitHub Latest 버전 비교, 자동 다운로드, 크기/SHA-256 검증, 게임 종료 대기, 정상 종료 후 자동 설치·재실행.
- 다운로드/검증/설치 준비 실패 시 현재 실행 유지, 설치 결과 저장 및 재시도 간격 적용.
- 사용자 요청으로 0.4.1 로컬 커밋. 공개 게시/tag/push는 별도 요청 범위.

## 0.4.0 — 2026-09-06

- 순위 타워를 각 참가자의 AMS2 최고 랩 기록 중심으로 변경. 연습·예선의 아웃랩 → 랩 타임 주행 중 → 인정 기록을 표시하고 기존 최고 기록은 유지
- 랩 카운터가 그대로인 첫 계측 시작, NotStarted 아웃랩, 일시정지/메뉴 전환의 출차 이력 초기화 문제 수정
- 레이스 첫 랩 아웃랩/다음 랩 레이스 중 표시, 시작 전 잘못된 무효 알림 억제, 실제 무효 신호의 개인 타임 동결/이벤트와 기존 유효 최고 기록 분리
- 차고/피트 진입/정차/출차의 PIT 배지 누락 수정. 전후방 후보를 피트/트랙별로 분리하고 피트 근거리 월드 좌표 보조 계산 사용
- 전후방 시간차/LAP 표시 칸 유지, 연습·예선의 LAP 차이 제거, 출발 전 위치 미설정 보조 처리
- 모든 보조 패널에서 비균등 글꼴 늘림 제거. 독립 패널 크기·균등 글꼴 배율 유지, Race Control 화면용 이력 숨김, 신규 이벤트 플래시
- 대기 화면의 정확한 세션명과 UI 전용 싱글/멀티 선택 추가(자동 감지 아님)
- 음수 Story 거리의 Compact 변환 실패 방어, 실패 청크 격리/원본 로컬 보존, 부분 저장 후 동일 바이트 재시도, restart witness/archive 식별 연결 개선
- 비밀을 제외한 저장 실패/403 진단, 403 영구 오류 격리와 잘못된 duplicate 성공 처리 방지. 기존 격리 큐 자동 재전송 없음
- Compact wire/schema/cadence/authority 정책 유지. Cafe24 서버 배포·API/DB 변경 없음. cadence PARTIAL/짧은 종료 경계 capture 및 실게임 120 FPS 보장은 해결 완료로 주장하지 않음

## 0.3.1 — 2026-09-05

- 타워 높이에서 누락된 테두리/여백 16px을 반영해 마지막 행 잘림 수정. 사용자 창 높이에 따라 2~64행을 즉시 표시하고 상위 순위 우선/플레이어 마지막 고정 유지
- 편집 미리보기에서도 크기 변경을 즉시 반영하고 용량 변경을 추월로 애니메이션하지 않음. 내용을 가리던 편집 상단 바 제거
- 보조 UI의 가로·세로 독립 크기 적용. Race Control은 별도 실제 크기 reflow로 글자 왜곡·잘림 방지, 낮고 넓은 창에서는 현재 알림 우선 배치
- 타이밍 갱신 시 행 객체/컨테이너 유지, 값이 바뀐 속성만 통지, 강제 UpdateLayout 제거, 데이터 투영을 Background 우선순위로 분리
- WPF 모션 144Hz 힌트와 연속 모션 retarget 적용. 실제 행 재정렬 프로브 추가(게임 FPS가 아닌 UI 좌표 갱신 측정, 게임 부하의 120 FPS 보장은 아님)
- Race Control의 snapshot/history ID 변경을 새 메시지로 취급하지 않아 같은 황색기 등장 효과 반복 억제
- 모든 참가자의 섹터 합계를 현재 랩 시간으로 사용하던 경로 제거. 각 차량의 라인 통과 관측 시계 `~`, AMS2 직전 랩 `L`, 로컬 직접값을 구분
- 관측 시계의 pause/stale/슬롯 재사용/역주행 wrap/teleport 초기화 및 개별 FIN/RET/DNF/DSQ 정지 검증. 시간제 세션 타이머가 0이어도 주행 중인 차량 시계는 유지
- Release build 0 warnings / 0 errors, Client 77/77, Activity 97/97. Compact Protocol/캡처/업로드/API/DB 변경 없음
